            ...
            if contador > 0:
                with transaction.atomic():
                    try:
                        ...
                        impuesto_id = (row[18]).strip()
                        itemunidadmedidastock_id = (row[19]).strip()
                        subtotal_detalle = (row[20]).strip()
                        ganancia = (row[21]).strip()
                        porcentaje_ganancia = (row[22]).strip()
                        costo = (row[23]).strip()
                        forma_pago = (row[24]).strip()
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            eFacturaVenta = FacturaVenta(valida=True, numero=numero,
                                                                         ...
                                                                         direccionentrega=direccionentrega,
                                                                         valorimpuesto=float(
                                                                             valorimpuesto.replace(',', '.')),
                                                                         efectivorecibido=float(
                                                                             efectivorecibido.replace(',', '.')),
                    ...
                    unidaddemedida = UnidadMedida.objects.get(id=11)
                    unidaddemedida = UnidadMedida.objects.filter(id=11)
                    unidaddemedidas = UnidadMedida.objects.filter(status=True).order_by('unidad')
                    unidadedemedidas_1 = UnidadMedida.objects.all().order_by('unidad')
                    ...
                    total_devies = \
                    ItemUnidadMedidaStock.objects.filter(itemunidadmedida__item__marca__id=9, status=True).aggregate(
                        total=Avg('stock'))['total']
                    facturasxfecha_promedio = FacturaVenta.objects.filter(status=True, fechafactura__gte='2023-05-01',
                                                                          fechafactura__lte='2025-05-10').aggregate(
                        venta=(Avg(F('total'), output_field=FloatField()))).get('venta')
                    facturaxfecha_suma = FacturaVenta.objects.filter(status=True, fechafactura__gte='2023-05-01',
                                                                     fechafactura__lte='2025-05-10').aggregate(
                        venta=(Sum(F('total'), output_field=FloatField()))).get('venta')
                    facturasxfecha = FacturaVenta.objects.filter(status=True, fechafactura__gte='2023-05-01',
                                                                 fechafactura__lte='2025-05-10')
                    facturasxcoincidencia = FacturaVenta.objects.filter(
                        Q(numero__icontains='001') | Q(codigo__icontains='001'), status=True)
                    facturasxincidencia = FacturaVenta.objects.filter(
                        Q(numero__icontains='001') | Q(codigo__icontains='001'), status=True, valida=True,
                        pagada=True).count()
                    ...
                    facturasdeventa = FacturaVenta.objects.filter(status=True).annotate(
                        es_efectivo=Exists(FacturaVentaFormaPago.objects.filter(status=True)))
